...
dogfood-docker: release
	docker build --build-arg TARGET=$(MUSL_TARGET) --tag is_affected --file Dockerfile .
	docker run --rm --volume $(PWD):/workspace --workdir /workspace $(GITHUB_ACTIONS_ENV) is_affected --help
